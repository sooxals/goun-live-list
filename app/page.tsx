    ...
    <main className="min-h-screen bg-[#F8F9FD] text-[#1D1D1F] pb-20 font-sans">
      <div className="max-w-5xl mx-auto px-4 pt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          {filtered.map((song) => (
            <div key={song.id} className="bg-white p-7 rounded-[2.5rem] shadow-sm flex items-center justify-between border border-white hover:border-indigo-50 hover:shadow-2xl transition-all duration-300">
              <div className="overflow-hidden flex-1 pr-4">
                <div className="flex items-center gap-2 mb-1.5">
                  <h3 className="font-black text-2xl truncate text-gray-950 tracking-tight">{song.artist}</h3>
                  <span className="px-2 py-0.5 bg-gray-100 text-gray-500 text-[10px] font-black rounded-lg uppercase tracking-tighter">
                    {song.genre}
                  </span>
                </div>
                <p className="text-gray-400 font-semibold text-base truncate ml-0.5">{song.title}</p>
              </div>
              
              {isAdminMode && (
                <div className="flex flex-col gap-2">
                  <button onClick={() => { setEditingSong(song); setFormArtist(song.artist); setFormTitle(song.title); setFormGenre(song.genre); window.scrollTo({top:0, behavior:'smooth'}); }} className="p-2.5 text-indigo-400 bg-indigo-50 rounded-xl">✏️</button>
                  <button onClick={async () => { if(confirm('삭제하시겠습니까?')) { await supabase.from('LIVE LIST').delete().eq('id', song.id); fetchSongs(); } }} className="p-2.5 text-red-400 bg-red-50 rounded-xl">🗑️</button>
                </div>
              )}
            </div>
          ))}
